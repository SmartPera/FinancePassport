...
// CHANGED: this used to read mobileIndex/emailIndex directly from
// the client, which required those paths to be publicly readable —
// even to people who never sign up, since this check runs before
// OTP verification. Now it calls a Cloud Function that does the
// same check server-side (via Admin SDK, which bypasses security
// rules) and returns only "mobile" | "email" | null — never the
// actual index contents. Same signature, same return value, so
...
export async function checkDuplicate(mobile, email) {
  ...
  try {
    const result = await checkFn({ mobileKey, emailKey });
    return result.data.duplicate; // "mobile" | "email" | null
  } catch (err) {
